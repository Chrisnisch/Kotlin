...
fun dateStrToDigit(str: String): String {
    val parts = str.split(" ")
    if (!str.matches(Regex("""\d+\s[а-я]+\s\d+"""))) return ""
    val d = parts[0].toInt()
    val m = months.indexOf(parts[1]) + 1
    val y = parts[2].toInt()
    if (m == 0 || daysInMonth(m, y) < d) return ""
...
fun dateDigitToStr(digital: String): String {
    ...
    val y = parts[2].toInt()
    if (daysInMonth(parts[1].toInt(), y) < d) return ""
...
//    val jumps = jumps.replace("[-%]".toRegex(), "").split(Regex("""\s+"""))
//    println(jumps)
//    return jumps.map { it.toInt() }.max()
}
...
fun plusMinus(expression: String): Int {
    ...
    var res = expression.first().toInt()
    var i = 1
    while (i < expression.size) {
        if (expression[i] == "+") res += expression[i + 1].toInt()
        else res -= expression[i + 1].toInt()
        i += 2
    }
    return res
}
